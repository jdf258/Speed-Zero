...
class Wifi_Packet:
    ...
    def serialize(self):
        return self.header + chr(len(self.payload)) + self.payload
...
class Wifi_Connection:
    ...
    def send(self, packet):
        ...
        logger.debug('send: ' +  binascii.hexlify(packet.serialize()))
    ...
    def recv(self, debug_filter = True):
        while True:
            ...
            if (packet.header == WIFI_DEBUG):
                logger.debug('Device: ' + packet.payload)
                if not debug_filter:
                    return packet
            else:
    ...
    def induction(self):
        ...
        while True:
            ...
            if(recv_packet.header == WIFI_INDUCTION_DATA):
                reading = ord(recv_packet.payload)
